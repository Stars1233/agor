...
Managed integrations are installed per Agor version, so upgrading Agor leaves the previous version's
integrations behind and the new version starts with none. Reinstall the tools you use, naming them
explicitly, and confirm the result:
...
Naming the tools is the form to script against. It depends on nothing Agor recorded previously, so
it behaves identically on a fresh machine, after skipping several versions, and in an
immutable-infrastructure image built from scratch.

On a machine that already had integrations installed, `--restore` reinstalls whatever the previous
version had, as a convenience for interactive upgrades:

```bash
agor install --restore
```

On a machine with nothing installed it reports that there is nothing to restore and exits
successfully, so it is safe to leave in an unattended script — but a fresh or rebuilt host has no
previous version to read, so provisioning must still name its tools.

Integrations from older versions are kept until you remove them. Reclaim that space with `--prune`,
which deletes managed directories belonging to every version except the one you are running:
...
### PostgreSQL upgrade through migration 0074

Migration `0074_knowledge_embedding_claims` changes the ownership protocol used by Knowledge embedding workers. Existing PostgreSQL installations must apply this migration as a **stop-the-world cutover**. Rolling old and new daemons against the same database during this one upgrade is not supported.
